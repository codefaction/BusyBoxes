 //<![CDATA[
  ...
  var Detuschoffs1 = [+2, +1, -1, -2, +2, +1, -1, -2];
  var Detuschoffs2 = [+1, +2, +2, +1, -1, -2, -2, -1];

  var Detuschoffs1_ = [+1, +2, -2, -1, +1, +2, -2, -1];
  var Detuschoffs2_ = [+2, +1, +1, +2, -2, -1, -1, -2];
      ...
      if (isRunning) {
          ...
          for (var i in cells) {
              ...
              if (((xyz[0] + xyz[1] + xyz[2]) & 1) == (frame & 1)) {                  // only operate on cells appropriate for this phase
                  ...
                  var flip = getDetuschMove(xyz, x1, x2);

         //         alert("flip value "+flip.toString());
                  if (flip) {
                 //   alert("flip happened");
                                         						flips.push([xyz, cell[1]]);
                   }
          ...
          for (i in flips) {
          //    alert("flips are here" + i.toString());
              var args = flips[i];
              flipCell(args[0], args[1]);
          }
  ...
  function getDetuschMove(xyz, x1, x2) {
      ...
      for (i = 0; i < 8; i++) {
          ...
          d2[x1] = Detuschoffs1_[i];
          d2[x2] = Detuschoffs2_[i];
          //alert( "was here" + i.toString());
...
/*                if (CELL_TRAIL) {
      ...
      if(_3cellstate[xyz])
      {
      //  alert(" [] = "+ _3cellstate[xyz]);

        var v = _3cellstate[xyz];
        var mesh;

       setObjPosition(mesh, xyz);
       delGrid(xyz);
        if(v ==  -1)
        {
         ...
         {
                  setMinusBrushPosition(xyz);

                          var voxel = new THREE.Mesh(cube, new THREE.MeshColorFillMaterial(Minuscolors[minuscolor]));
                          scene.removeObject(mesh);
                          delGrid(xyz);
                          putMinus1Grid(voxel, xyz);
                          setObjPosition(voxel, xyz);
                          voxel.overdraw = true;
                          scene.addObject(voxel);
  ...
  function setBrushPosition(xyz) {
      ...
      if ((xyz[0] + xyz[1] + xyz[2]) % 2 == 0) {
         ...
         // if (_3cellstate[xyz])
          //    if(_3cellstate[xyz]== )
          setMinusBrushColor(0);
          setBrushColor(0);                    
      }
      else {
          setBrushColor(5);
          setMinusBrushColor(1);               
      }
      document.getElementById("cursorpos").innerHTML = xyz[0] + "," + xyz[1] + "," + xyz[2]
  }
  function setMinusBrushPosition(xyz) {
      setObjPosition(brush, xyz);
      if ((xyz[0] + xyz[1] + xyz[2]) % 2 == 0) {
          setMinusBrushColor(0);
      }
  ...
  function putMinus1Grid(obj, xyz) {
      grid[xyz] = obj;
      _3cellstate[xyz] = -1;
      //alert("set to -1");
  ...
  function get3CellGrid(xyz) {
      ...
      return _3cellstate[xyz]
  }

  function delGrid(xyz) {
      delete grid[xyz];
      delete _3cellstate[xyz];
      //alert("set to 0");
...
//if(DEBUG) console.log("key:", event.keyCode);
      ...
      switch( event.keyCode ) {
          ...
          case 13:
              ...
              if (obj) {
                  if (_3cellstate[cursor]) {
                      if (_3cellstate[cursor] == -1) {
...
if (event.detail > 0) {
  ...
  function setMinusBrushColor(value) {
      minuscolor = value;
      //brush.material[0].color.setHex(Minuscolors[color] ^ 0x4C000000);
